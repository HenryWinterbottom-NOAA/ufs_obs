...
import re
from tools import parser_interface, fileio_interface, datetime_interface
...
__all__ = ["dateinfo", "decode", "layer_mean", "locations", "obslocation", "sfcpres", "tempdrop", "write_hsa"]
...
def decode(tempdrop_obj: SimpleNamespace) -> SimpleNamespace:
    ...
    for msgstr in tempdrop_obj.tempdrop:
        if "xx" in msgstr.lower():
            [drop(luidx, 1, iflag, tempdrop_obj.dateinfo.year_short,
